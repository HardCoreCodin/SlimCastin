...
#define CAMERA_DEFAULT__FOCAL_LENGTH 1.0f
#define CAMERA_DEFAULT__TARGET_DISTANCE 10

#define NAVIGATION_DEFAULT__MAX_VELOCITY 1.5
...
struct Pixel {
    ...
    INLINE_XPU Pixel& operator = (const Pixel& other) {
        color = other.color;
    ...
    INLINE_XPU Pixel& operator += (f32 factor) {
        color += factor;
        return *this;
    }
...
struct Slice {
    ...
    INLINE_XPU T& operator[] (int row) const { return data[row]; }
};

#define ARRAY_SIZE(array) (sizeof(array) / sizeof(array[0]))
#define iterSlice(slice, item, index) item = &(slice).data[0]; for (u32 index = 0; index < (slice).size; index++, item++)
#define setSliceToStaticArray(slice, static_array) slice.data = &((static_array)[0]); slice.size = ARRAY_SIZE(static_array)
#define setSliceToOtherSlice(slice, other_slice) slice.data = other_slice.data; slice.size = other_slice.size
#define setSliceToRangeOfStaticArray(slice, static_array, start, end) setSliceToStaticArray(slice, static_array); slice.data += start; slice.size = (end) - (start)

#define SliceFromStaticArray(T, array) Slice<T>{&array[0], ARRAY_SIZE(array)}
...
void resizeGrid(Grid<CellType>& grid, u16 width, u16 height) {
    ...
    if (grid._rows.data) delete[] grid._rows.data;
    grid._rows.size = height;
    grid._rows.data = new Slice<CellType>[height];

    for (i32 y = 0; y < height; y++) {
        grid._rows[y].size = width;
        grid._rows[y].data = grid._cells.data + width*y;
    }
    setSliceToOtherSlice(grid.cells, grid._rows);
}
