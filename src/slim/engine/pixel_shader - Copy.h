...
//
//
// // Simple integer hash function (fast, not cryptographic)
// INLINE_XPU u32 hash(u32 x) {
//     x ^= x >> 16;
//     x *= 0x7feb352d;
//     x ^= x >> 15;
//     x *= 0x846ca68b;
...
// // INLINE_XPU float lerp(float a, float b, float t) {
// //     return a + t * (b - a);
// // }
//
// // Pseudo-random value in [-1, 1] from integer coordinates
// INLINE_XPU float randomValue(int x, int y, int z, int w) {
//     u32 h = hash((u32)x);
//     h = hash(h ^ (u32)y);
//     h = hash(h ^ (u32)z);
//     h = hash(h ^ (u32)w);
...
//
// // 4D value noise
// float noise4D(const vec3& P, float w) {
...
//     float s = fade(fw);
//
//     // Interpolate over 16 corners of the 4D hypercube
//     float n0000 = randomValue(X0, Y0, Z0, W0);
//     float n1000 = randomValue(X1, Y0, Z0, W0);
//     float n0100 = randomValue(X0, Y1, Z0, W0);
//     float n1100 = randomValue(X1, Y1, Z0, W0);
//     float n0010 = randomValue(X0, Y0, Z1, W0);
//     float n1010 = randomValue(X1, Y0, Z1, W0);
//     float n0110 = randomValue(X0, Y1, Z1, W0);
//     float n1110 = randomValue(X1, Y1, Z1, W0);
//     float n0001 = randomValue(X0, Y0, Z0, W1);
//     float n1001 = randomValue(X1, Y0, Z0, W1);
//     float n0101 = randomValue(X0, Y1, Z0, W1);
//     float n1101 = randomValue(X1, Y1, Z0, W1);
//     float n0011 = randomValue(X0, Y0, Z1, W1);
//     float n1011 = randomValue(X1, Y0, Z1, W1);
//     float n0111 = randomValue(X0, Y1, Z1, W1);
//     float n1111 = randomValue(X1, Y1, Z1, W1);
//
//     // Interpolate in 4D
...
INLINE_XPU void setUVBySphere(f32 X, f32 Y, f32 Z, f32 *u, f32 *v) {
...
INLINE_XPU bool intersectSphere(const vec3 &origin, vec3 &direction, f32 &hit_distance, f32 *out_squared_distance_to_center = nullptr) {
    ...
    INLINE_XPU bool prepare(
        ...
        if (render_state.render_mode == RenderMode_Beauty) {
            ...
            for (u8 i = 1; i < (render_state.light_count + render_state.enemy_count); i++) {
                ...
                if (sphere_tracer.prepare(point_light_position, Ro, V)) {
                    LP = V * sphere_tracer.proj - sphere_tracer.RoC;
                    ...
                    if (i < render_state.light_count) {
                        ...
                        if (sphere_tracer.hit(radius, one_over_radius, hit_distance)) {
                            ...
                            LP = Ro - point_light_position;
                            vec3 step = V * radius;
                            vec3 near = LP + step * sphere_tracer.t_near;

                            // flare_light = render_state.noise(near.normalized() * 3.0f);
                            // continue;
                            vec3 far = LP + step * sphere_tracer.t_far;
                            f32 fraction = 1.0f / render_state.step_count;
                            f32 range_step = (sphere_tracer.t_far - sphere_tracer.t_near) * fraction;
                            sphere_tracer.t_far = range_step;
                            step = (far - near) * fraction;
                            LP = near + step * 0.5f;
                            f32 accum = 0.0f;
                            for (u8 k = 0; k < render_state.step_count; k++, sphere_tracer.t_far += range_step, sphere_tracer.t_near += range_step, LP += step) {
                                // f32 density = sphere_tracer.integrateDensity();
                                // density *= density;
                                // density *= density;
                                // density *= density;
                                // density *= density;
                            ...
                            // radii = screen_ratio * sphere_tracer.proj;
                            // radii.y += point_light_position.y;
                            //
                            // radii += render_state.time * 2.0f;
                            // radii.x *= 13.0f;
                            // radii.y *= 9.0f;
                            // radii = radii.sin();
                            // radius = 0.5f - (radii.x * radii.y) * 0.04f - 0.04f;


                            // flare_light = accum;
                            // continue;

                            // sphere_tracer.hit(radius, 1.0f / radius, hit_distance);
                            // f32 flare_intensity = sphere_tracer.integrateDensity();
                            // flare_intensity *= flare_intensity;
                            // flare_intensity *= flare_intensity;
                            // // flare_intensity *= flare_intensity;
                            // // flare_intensity *= flare_intensity;
                            ...
                            // // flare_intensity *= flare_intensity;
                            // // flare_intensity *= flare_intensity;
                            // flare_intensity *= 20.0f;

                            flare_light += 0.35f * Color(-0.25f).lerpTo(enemy.color * (enemy.intensity * accum), distance);
                            ...
                            radii *= 9.0f;
                            // f32 displacement_amount = 1.0f+V.dot(near);
                            // displacement_amount *= displacement_amount;
                            // displacement_amount *= displacement_amount;
                            // displacement_amount *= displacement_amount;
                            // flare_light = 0.0f;//displacement_amount;
                            // flare_light = displacement_amount;
                            // displacement_amount *= 8.0;
                            // displacement_amount = 0.0f;
                            ...
                            // // Rd.y *= enemy.sincos.x;
                            //
                            // vec3 ray_origin = Ro - point_light_position;
                            // // ray_origin.y *= 2.0f;
                            // // ray_origin.x *= 5.5f;
                            // // ray_origin.z *= 5.5f;
                            //
                            // f32 distance_to_center_squared;
                            // if (intersectSphere(ray_origin, Rd, hit_distance, &distance_to_center_squared)) {
                            //     LP = ray_origin + Rd * hit_distance;
                            //     LP = LP.normalized();
                            //     f32 fresnel = -LP.dot(V);
                            //     fresnel *= fresnel;
                            //     fresnel *= fresnel;
                            //     fresnel *= fresnel;
                            //     fresnel *= fresnel;
                            ...
                            //     LP *= 0.5f;
                            //     LP += 0.5f;
                            //     flare_light = LP.toColor();
                            // }
                        }
                    }
                // }

                    // f32 max_distance = hit_distance;


                    // if (sphere_tracer.prepare(point_light_position, vec3{Ro.x * 0.5f, Ro.y, Ro.z * 0.5f}, Rd)) {
                    //     if (sphere_tracer.hit(1.0f, 1.0f, max_distance)) {
                    //         LP = vec3{Ro.x * 0.5f, Ro.y, Ro.z * 0.5f} + Rd * sphere_tracer.t_near - enemy.position;
                    //         LP = LP.normalized();
                    //         LP *= 0.5f;
                    //         LP += 0.5f;
                    //         flare_light = LP.toColor();
                ...
                //
                //     f32 radius = 0.5f - (radii.x * radii.y) * 0.02f - 0.02f;
                //     if (sphere_tracer.hit(radius, 1.0f / radius, hit_distance)) {
                ...
                //         flare_intensity *= flare_intensity;
                //         flare_intensity *= flare_intensity;
                //         flare_intensity *= 4.0f;
                //
                //         flare_light += 0.35f * Color(-0.25f).lerpTo(enemy.color * (enemy.intensity * flare_intensity), 1.0f);
                //     }
                // }
                        //
                        // LP *= 0.65f;
                        ...
                        //         if (sphere_tracer.hit(radius, 1.0f / radius, max_distance)) {
                        //             i32 X = (i32)(radii.x * 6.0f) + (i32)x;
                        //             X = X < 0 ? 0 : (X >= render_state.screen_width ? (i32)(render_state.screen_width - 1) : X );
                        //             x = (u16)X;
                        //
                        ...
                        //             flare_intensity *= 4.0f;
                        //
                        //             LP = Ro - V * (sphere_tracer.proj - sqrt(radius - distance)) - point_light_position;
                        //             LP.x = 1.0f / LP.squaredLength();
                        //             LP.x *= LP.x;
                        //             LP.x *= LP.x;
                        //
                        ...
                        //             distance = 1.0f - distance;
                        //
                        //             flare_light += 0.35f * Color(-0.25f).lerpTo(enemy.color * (enemy.intensity * flare_intensity), distance);
                        //         }
                        //
