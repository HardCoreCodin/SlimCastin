...
// (Pre)Swizzle N.z <-> N.y
INLINE_XPU quat getNormalRotation(const vec3 &N, f32 magnitude = 1.0f                     ) {
...
INLINE_XPU quat convertNormalSampleToRotation(const Color &normal_sample, f32 magnitude   ) {
    return getNormalRotation(decodeNormal(normal_sample), magnitude);
}
...
INLINE_XPU vec3 rotateNormal(const vec3 &Ng, const Color &normal_sample, f32 magnitude) {
