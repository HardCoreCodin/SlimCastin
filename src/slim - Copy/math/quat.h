...
struct quat {
    ...
    INLINE_XPU vec3 operator * (const vec3 &v) const {
        vec3 result{axis.cross(v)};
        vec3 qqv{axis.cross(result)};
        result = result.scaleAdd(amount, qqv);
        result = result.scaleAdd(2, v);
        return result;
    }
